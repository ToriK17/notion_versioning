...
## Deterministic vs. Non-deterministic Encryption (Rails-style)

### 🔒 Non-deterministic encryption

- **Default in Rails 7.1+ with `encrypts :field`**
- **Non-deterministic encryption = secure randomness every time.**
- Uses AES-GCM + a **random IV (initialization vector)** each time you encrypt
- **Same plaintext results in different ciphertext every time**
- **Very secure**, but **you can't search or index the data**

### 🕵️ Deterministic encryption

- **Optional setting in Rails**
- **searchable = deterministic**
- Same plaintext → same ciphertext
- Enables exact-match lookups (aka: `find_by :auth_token`)
- **Searchable**, but **less secure**: if two users have the same value, it will be obvious at the encrypted level (e.g., duplicated tokens or email addresses)

### `blind_index` for Encrypted Lookups

Now when recalling this information you can’t just: `User.find_by(auth_token: token)` 

Because Rails is just sitting there, like: “Sorry, can’t find what I can’t read.” 🔒🙈
This is because Active Record **can’t search on encrypted fields** unless you give it a *searchable version* (aka: a blind index)

**What is a blind index?**
It’s a **deterministic hash** of a value that can be indexed and queried *without revealing the actual data.*
This allows you to do `find_by` look ups by the token without having to decrypt every row to find the match.

`

`bin/rails generate active_record:encryption:add_deterministic_key_columns <table> <token_column>` 
